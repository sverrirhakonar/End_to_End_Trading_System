...
class SimulatedMatchingEngine:
    ...
    def process_order(self, order: dict, current_tick: 'pd.Series'):
        ...
        if random.random() < self.fill_reject_chance:
            ...
            # Note: We give the logger the *original* order dict
            self.order_logger.log_event('REJECTED', order, reason="Random engine rejection")
            return [] # No fills
    ...
    def _fill_market_order(self, order: dict, current_tick: 'pd.Series'):
        ...
        if random.random() < self.partial_fill_chance:
            ...
            fill_qty = int(order['qty'] * random.uniform(0.1, 0.9))
            print(f"MatchingEngine: Order partially filled ({fill_qty} / {order['qty']})")
    ...
    def _process_limit_order(self, order: dict, current_tick: 'pd.Series'):
        ...
        # --- Can't fill, so add to waiting room ---
            ...
            # We get back the order (now with its new ID)
            order_with_id = self.order_book.add_order(order)
            
            # Log that it's been "placed" (but not filled)
            self.order_logger.log_event('PLACED', order_with_id, reason="Placed in book to wait")
            return [] # No fills
    ...
    def check_open_orders(self, current_tick: 'pd.Series'):
        ...
        while True:
            ...
            if best_bid_order['price'] >= current_tick['Low']:
                print(f"MatchingEngine: Waiting BUY order {best_bid_order['id']} FILLED.")
                
                # Pop it from the book
                popped_order = self.order_book.pop_best_bid_order()
                fill_price = popped_order['price']
                fill_qty = popped_order['qty'] # We fill 100% from the book
                
                self.order_logger.log_event('FILLED', popped_order, fill_qty=fill_qty, fill_price=fill_price)
                fills.append({
                    'side': 'BUY',
                    'fill_qty': fill_qty,
                    'fill_price': fill_price
        ...
        while True:
            ...
            if best_ask_order['price'] <= current_tick['High']:
                print(f"MatchingEngine: Waiting SELL order {best_ask_order['id']} FILLED.")
                
                popped_order = self.order_book.pop_best_ask_order()
                fill_price = popped_order['price']
                fill_qty = popped_order['qty']
